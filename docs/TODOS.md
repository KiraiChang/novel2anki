...
- [x] **CEFR 字庫中文批次翻譯 `--prefetch-cefr-zh`**（`src/nlp/cefrPrefetcher.ts`、`src/index.ts`，2026-06-08；**擴充 word_zh 直翻 2026-06-12**）：對 `word-cache.json` 中的英文定義批次翻譯為繁體中文（`definition_zh`），並額外執行 word_zh 批次——對每個缺少 `word_zh` 的 CEFR 詞直接翻譯 lemma 本身（如 `bridge` → `橋樑`），結果存入 `word-cache-zh.json`；即使該詞無英文定義也會翻譯 `word_zh`。已快取的詞自動跳過，需先執行 `--prefetch-cefr`。
...
- [x] **`--translate` 補齊所有空白欄位（`definition_en` / `definition_zh` / `context_sentence_zh`）**（`src/csv/beginnerTranslator.ts`，2026-06-11）：`needTranslation` 過濾條件擴充為三欄 OR（三者皆有才跳過）；Phase 3 新增 `definition_en` 回寫（本次從 API 取得才寫，原本有值不覆寫）；`estimateBeginnerTranslate` 費用估算同步更新。順帶修正 `src/index.ts` 遺留的 `--deepl-force` 提示文字 → `--translate-force`。
- [x] **各 CSV 缺漏統計表（HTML）**（`src/html/beginnerStatsExporter.ts`、`src/index.ts`，2026-06-11）：HTML 統計頁新增「各 CSV 缺漏統計表」區段，按來源 CSV 彙整每個檔案的詞彙數與三類缺漏計數（非零值橘色標示）。
- [x] **缺漏清單改為 per-source-CSV JSON，移除 `--missing-split`**（`src/html/beginnerStatsExporter.ts`、`src/index.ts`，2026-06-12）：移除 `--missing-split <N>` CLI 旗標，改為在每個有缺漏的 source CSV 旁自動產生 `{basename}-missing.json`（缺 definition_en / definition_zh / word_zh）與 `{basename}-missing-sentence.json`（缺 context_sentence_zh，key = fnv1a hash），格式分別對應 `missing-cefr-words.json` 與 `sentence-cache.json`。`BeginnerStatsExportResult` 改為 `{ missingJsonPaths, missingSentenceJsonPaths }`（各為 `string[]`）。
- [x] **Vocab 字卡正面加 `definition_en`、背面加 `word_zh`**（`src/anki/exporter.ts`、`src/html/exporter.ts`、`src/cards/templates.ts`、`src/cards/types.ts`，2026-06-12）：`VocabCard` 新增 `definition_en?` 與 `word_zh?` 兩個選用欄位；Anki 字卡正面附英文定義（若有），背面先顯示中文對應詞再顯示定義；HTML flash 預覽同步更新。
- [x] **`--prefetch-cefr-zh` 分離追蹤 `missingZhWords` 與 `missingWordZhWords`**（`src/nlp/cefrPrefetcher.ts`，2026-06-12）：`PrefetchZhResult` 拆出獨立的 `missingWordZhWords[]`（無 word_zh）與 `missingZhWords[]`（無 definition_zh），兩者各自追蹤，不再互相影響；`--prefetch-cefr-zh` CLI 輸出亦分別報告兩項缺漏數。
...
- [x] **跨書英文定義與中文翻譯 SQLite 快取 `word-def.db`**（`src/nlp/wordDefDb.ts`、`src/csv/wordDefDbSync.ts`、`src/index.ts`，2026-06-16）：新增獨立於 `word-cache.json` / `word-cache-zh.json` 的 SQLite 定義快取，key = `word:pos::fnv1a(definition_en)`（hash 英文定義確保同一 MW 定義跨書共用同一翻譯），schema `key / en / zh / source`（`source` 記錄如 `deepl`）；`--csv-to-def-db`：把 CSV 已有翻譯（`definition_zh` 非空）寫入 SQLite，跳過空白列與已存在 key；`--def-db-to-csv`：從 SQLite 補填 CSV 中空白的 `definition_zh` 與 `definition_zh_source`。啟動時自動 migration 補 `source` 欄（向下相容舊 DB）。

- [ ] **WSD tokenizer 升級路徑評估**（`scripts/wsd/`）：目前以 `use_fast=False`（Python slow tokenizer）繞開 `tokenizers` 0.20.x Rust 層型別驗證問題。長期可考慮：① 釘版 `tokenizers<0.20` 並同步降版 `transformers`（需確認 sentence-transformers 3.x 相容範圍）；② 等待 `sentence-transformers` 修復與 `tokenizers` 0.20+ 的相容問題後，回歸 `SentenceTransformer.encode(sentences, prompt=BGE_QUERY_PREFIX)` 寫法（3.x 官方推薦的 prompt 參數用法）。Slow tokenizer 在 92 items 批次下速度尚可接受，100–500 詞規模無需急於切換。
...
- [x] **`word_zh` 對應詞欄位**（`src/nlp/wordCache.ts`、`src/csv/beginnerExporter.ts`、`src/csv/beginnerTranslator.ts`、`src/nlp/cefrPrefetcher.ts`，2026-06-12）：`CacheZhEntry` 新增 `word_zh?: string; word_zh_source?: string`（直接對應的中文單字，如「橋樑」），`WORDS_HEADERS` 新增 `word_zh`、`word_zh_source` 欄。`--translate` 新增 Phase 2.5 批次翻譯 lemma 取得 `word_zh`（快取優先），填入後立即以 `setWordZhIfEmpty` 存入 `word-cache-zh.json`；`--fill-def-zh` 亦包含 `word_zh` 雙向同步；`--prefetch-cefr-zh` 定義翻譯結束後額外執行 word_zh 批次，對無 `word_zh` 快取的 CEFR 詞直譯 lemma（無英文定義的詞亦翻譯）。`WordCacheManager` 新增 `getWordZh` / `getWordZhSource` / `setWordZhIfEmpty`。舊 CSV（無 `word_zh` 欄）執行 `--translate` 時自動插入欄位，向後相容。
