...
- [x] **`word_zh` 對應詞欄位**（`src/nlp/wordCache.ts`、`src/csv/beginnerExporter.ts`、`src/csv/beginnerTranslator.ts`，2026-06-12）：`CacheZhEntry` 新增 `word_zh?: string; word_zh_source?: string`（直接對應的中文單字，如「橋樑」），`WORDS_HEADERS` 新增 `word_zh`、`word_zh_source` 欄。`--translate` 新增 Phase 2.5 批次翻譯 lemma 取得 `word_zh`，填入後立即以 `setWordZhIfEmpty` 存入 `word-cache-zh.json`；`--fill-def-zh` 亦包含 `word_zh` 雙向同步。`WordCacheManager` 新增 `getWordZh` / `getWordZhSource` / `setWordZhIfEmpty`。舊 CSV（無 `word_zh` 欄）執行 `--translate` 時自動插入欄位，向後相容。
