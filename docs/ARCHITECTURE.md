...
beginnerExporter.ts
  ...
  ├─ *-beginner-words.csv（14 欄，含 definition_en / word_zh 預查欄）  ← --beginner
  ├─ *-beginner-words-part-NN.csv（14 欄，分割版）                     ← --beginner-split N
  ...
  [選用] beginnerTranslator.ts（--mw / --translate / --translate-force / --update-dict）
    ...
    needTranslation 過濾條件：definition_en 空白 OR definition_zh 空白 OR context_sentence_zh 空白 OR word_zh 空白（四者皆有才跳過）
  ...
  [CSV 輸入 → 字卡輸出路徑（已翻譯 CSV 直接匯出）]
    computeBeginnerWordStats(csvPaths)
      ...
      （讀 lemma / pos / cefr_level / coverage_rank / global_frequency /
         ...
         word_zh / word_zh_source，依 coverage_rank 排序）
      WordStat 另記 sourcePath（來源 CSV 完整路徑）與 sourceFile（basename，供 HTML 顯示）

    exportBeginnerStatsToHtml(stats, deckName, outputDir)
      → BeginnerStatsExportResult { htmlPath, missingJsonPaths, missingSentenceJsonPaths }
        兩個 paths 欄位均為 string[]；每個有缺漏詞 / 缺漏例句的 source CSV 各貢獻一個路徑
      同時寫出 1 + 2×M 個檔案（M = 有缺漏的 source CSV 數）：
        *-beginner-stats.html（自含式 HTML）
        ...
        {sourcePath-without-.csv}-missing.json（緊鄰各 source CSV；有缺 definition_en / definition_zh / word_zh 任一的詞才產生）
          格式：{ generated, missing_en_count, missing_zh_count, missing_word_zh_count, words: [{word, missing[], definition_en, definition_zh, word_zh}] }
          missing[] 元素：'en'（缺 definition_en）/ 'zh'（缺 definition_zh）/ 'word_zh'（缺 word_zh）
        {sourcePath-without-.csv}-missing-sentence.json（緊鄰各 source CSV；有缺 context_sentence_zh 的詞才產生）
          格式：{ fnv1a(context_sentence): { en, zh: "", source: "" } }（key 同 sentence-cache.json）
```
...
| 詞彙匯入 | `src/csv/beginnerImporter.ts` | 偵測 CSV 格式（words/tokens）、支援多檔合併 → VocabCard[]（含 `definition_en?` 與 `word_zh?`）；`definition_zh` 為空的列仍合併（不過濾）；`computeBeginnerWordStats()` → `BeginnerWordStats`（含 per-word `WordStat[]`，每筆附所有翻譯欄位及來源欄位、`sourcePath`（完整路徑）與 `sourceFile`（basename）；整體附 `missingDefEn`、`missingContextZh` 計數） |
| 字彙統計 HTML | `src/html/beginnerStatsExporter.ts` | `BeginnerWordStats` → `BeginnerStatsExportResult { htmlPath, missingJsonPaths, missingSentenceJsonPaths }`（兩個 paths 均為 `string[]`）；HTML 含 sticky 導覽 + **各 CSV 缺漏統計表** + 3 張缺漏明細表 + 完整列表；每個有缺漏詞的 source CSV 旁自動產生 `{basename}-missing.json`（缺 definition_en / definition_zh / word_zh，key=fnv1a）與 `{basename}-missing-sentence.json`（缺 context_sentence_zh） |
...
| WSD 橋接層 | `src/csv/wsdClient.ts` | `--wsd` 語意消歧的 TypeScript → Python 橋接；以 `child_process.execFile` 呼叫 venv Python，stdin 送 JSON 陣列（`{ word, shortdefs[], sentence }`），解析 stdout JSON（`{ word, chosenIndex, score }`）；找不到 venv Python 時印出設定說明並 fallback 到系統 Python；Python 執行失敗時全部 fallback 到 `chosenIndex: 0, score: 0` |
| WSD shortdefs DB | `src/nlp/wsdShortdefsDb.ts` | MW shortdefs 的 SQLite 快取（`wsd-shortdefs.db`，`better-sqlite3`）；key = word（小寫，不含 POS），一次存入所有 POS 的原始 shortdefs，排序於讀取時依 `targetPOS` 動態套用（`applyPosOrder`）；`hashShortdefs()` 以 FNV-1a hash shortdefs 字串，供 `wsd-cache.json` 版本失效偵測；singleton `getWsdShortdefsDb()` |
| 定義快取 DB | `src/nlp/wordDefDb.ts` | MW 英文定義與中文翻譯的跨書 SQLite 快取（`word-def.db`，`better-sqlite3`）；key = `word:pos::fnv1a(definition_en)`（hash 英文定義而非例句，使同一 MW 定義在不同書共用同一翻譯）；schema：`key / en / zh / source`（`source` 記錄翻譯來源，如 `deepl`）；`set()` INSERT OR IGNORE（key 已存在整筆跳過）；`setZhIfEmpty()` 只在 zh 為空時補填並同步更新 source；啟動時自動 migration 補 `source` 欄（`ALTER TABLE ADD COLUMN`，已存在則靜默忽略）；singleton `getWordDefDb()` |
| 定義快取同步 | `src/csv/wordDefDbSync.ts` | CSV ↔ word-def.db 雙向同步；`syncCsvToWordDefDb(csvPath)`：讀 `definition_en`/`definition_zh`/`definition_zh_source`，跳過 `definition_zh` 為空的列，key 已存在時 INSERT OR IGNORE；`syncWordDefDbToCsv(csvPath)`：對 `definition_zh` 空白且 `definition_en` 非空的列查詢 DB，命中時補填 `definition_zh` 與 `definition_zh_source`，有填入才重寫 CSV |
...
interface VocabCard      { type: 'vocab'; word: string;
                           definition_en?: string;   // 英文定義（初學者模式，顯示於正面）
                           word_zh?: string;         // 單字中文翻譯（初學者模式，顯示於背面）
...
interface WordStat {
  ...
  word_zh: string;              word_zh_source: string;
  sourcePath: string;   // 來源 CSV 完整路徑（供產生 per-CSV missing 檔路徑用）
  sourceFile: string;   // 來源 CSV 檔名 basename（供 HTML 顯示用）
}
...
interface BeginnerStatsExportResult {   // beginnerStatsExporter.ts
  htmlPath: string;
  missingJsonPaths: string[];           // 每個有缺漏詞的 source CSV 旁產一個 {basename}-missing.json
  missingSentenceJsonPaths: string[];   // 每個有缺漏例句的 source CSV 旁產一個 {basename}-missing-sentence.json
}
```
...
| 檔案 | 來源 | 說明 |
|------|------|------|
| `wsd-shortdefs.db` | `--wsd` 執行時自動建立 | MW shortdefs SQLite 快取（`better-sqlite3`）；key = word，value = 所有 POS entries JSON；同一單字跨書只查一次 MW API |
| `wsd-cache.json` | `--wsd` 推論成功時自動寫入 | WSD 消歧結果快取；key = `word:pos::fnv1a(sentence)`（8 位 hex）；value = `{ chosenIndex, score, shortdefsHash }`；score = 0 時不寫入；shortdefsHash 不符時視為失效自動重推 |
| `word-def.db` | `--csv-to-def-db` 寫入 | MW 英文定義與中文翻譯的跨書 SQLite 快取；schema：`key TEXT PRIMARY KEY, en TEXT, zh TEXT, source TEXT`；key = `word:pos::fnv1a(definition_en)`；`source` 記錄翻譯來源（如 `deepl`）；同一 MW 定義在不同書共用同一筆翻譯，不因書籍不同重複翻譯 |
