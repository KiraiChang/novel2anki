...
- **初學者字彙統計 HTML 為自含式靜態頁面，同時在各 source CSV 旁產生缺漏 JSON 檔**（2026-06-12）：`exportBeginnerStatsToHtml(stats, deckName, outputDir)` 回傳 `BeginnerStatsExportResult { htmlPath, missingJsonPaths, missingSentenceJsonPaths }`（兩個 paths 均為 `string[]`）。HTML 不依賴外部資源，可離線瀏覽；頁面包含：sticky 導覽列（三個缺漏 section 跳轉按鈕 + 缺漏數 badge）、5 張摘要卡片、CEFR 長條圖、**各 CSV 缺漏統計表**（來源檔名 / 詞彙數 / 缺 definition_zh / 缺例句中文 / 缺 definition_en，非零值橘色標示）、**未翻譯單字**（缺 definition_zh）、**未翻譯例句**（缺 context_sentence_zh）、**未翻譯英文解釋**（缺 definition_en）三張缺漏表，以及完整詞彙列表；右下角浮動「回到頂端」按鈕（捲動 > 300px 顯示）。缺漏 JSON 採 per-source-CSV 設計（見下一條 NOTE），每個有缺漏的 source CSV 旁各產一組檔案。排序 / 搜尋邏輯以共用 `initTable()` 實作，數字欄 `parseFloat`、字串欄 `localeCompare('zh-TW')`。

- **缺漏清單改為 per-source-CSV JSON 檔，取代全域分割 CSV**（2026-06-12）：舊設計輸出三個全域缺漏 CSV（`{slug}-missing-def-zh.csv`、`{slug}-missing-ctx-zh.csv`、`{slug}-missing-def-en.csv`），並以 `--missing-split N` 分割。新設計改為：按 `WordStat.sourcePath` 分組，每個 source CSV 各自產生兩個 JSON 檔，緊鄰在 source CSV 旁邊：① `{csv-basename-without-.csv}-missing.json`（缺 `definition_en`、`definition_zh` 或 `word_zh` 任一的詞；格式與 `output/missing-cefr-words.json` 相同，含 `generated`、`missing_*_count`、`words[]` + `missing[]` 陣列）；② `{csv-basename-without-.csv}-missing-sentence.json`（缺 `context_sentence_zh` 的詞；格式與 `sentence-cache.json` 相同，key = `fnv1a(context_sentence)`，value = `{en, zh:"", source:""}`）。優點：缺漏檔與 source CSV 同目錄，易於一起管理；key 與全局快取相同，人工填好後可直接合入 `sentence-cache.json`。`--missing-split` 選項已移除。
...
- **`word_zh` 欄位設計：以 bare lemma 翻譯取得直接中文對應詞，與定義翻譯分離；先查快取再呼叫 API**（`src/csv/beginnerTranslator.ts`、`src/nlp/wordCache.ts`、`src/nlp/cefrPrefetcher.ts`，2026-06-12）：`word_zh`（如「橋樑」）與 `definition_zh`（如「一種橫跨河流的建築」）並存，前者為直接對應詞、後者為說明性定義。Phase 2.5 直接送 lemma（如 `bridge`）給翻譯後端，不附加 POS，因為翻譯後端對單一詞彙的上下文感知已足夠（實測 `bridge` → `橋樑`、`run` → `奔跑`）。此批次獨立於 Phase 2 的「定義 + 例句交錯批次」，避免干擾定義翻譯的語境。**快取優先**：Phase 2.5 準備時先呼叫 `wc.getWordZh(lemma, pos)` 查全局快取，命中的詞直接寫回 CSV（不呼叫翻譯 API），只有快取未命中的詞才加入 API 批次；`--translate-force` 跳過快取檢查強制重譯。API 翻譯後以 `setWordZhIfEmpty` 寫入快取（已有不覆蓋）。`--fill-def-zh` 亦可在不呼叫 API 的情況下從快取補填 `word_zh`（cache→CSV 方向）。`word_zh` 快取只存全局（`word-cache-zh.json`）、不分 domain/book，因為詞彙的直接中文對應是語言通用知識，不隨書籍/領域變化。舊 CSV 執行 `--translate` 時自動插入 `word_zh`、`word_zh_source` 欄（向後相容）。**`--prefetch-cefr-zh` 擴充**：`prefetchCefrZhToWordCache` 在完成定義翻譯批次後，額外執行 word_zh 批次：對每個 `getWordZh(word, null)` 為空的 CEFR 詞直接翻譯 lemma，結果以 `setWordZhIfEmpty` 存入 `word-cache-zh.json`。word_zh 批次與定義翻譯批次獨立執行：**即使該詞沒有英文定義（no-en），word_zh 仍會被翻譯**（定義翻譯需要 word-cache.json 有英文定義，但 word_zh 直接翻譯 lemma，不依賴英文定義）。

- **初學者模式 Vocab 字卡正面加入 `definition_en`，背面加入 `word_zh`**（`src/anki/exporter.ts`、`src/html/exporter.ts`、`src/cards/templates.ts`，2026-06-12）：`VocabCard` 新增兩個選用欄位（`definition_en?`、`word_zh?`），初學者模式 CSV 匯入時填入。Anki 字卡正面顯示單字 + 英文定義（`<div class="definition-en">`，若有），背面先顯示中文對應詞（`<div class="word-zh">`，若有），再顯示中文定義 + 例句。HTML 預覽頁（`--flash`）同步更新：翻面前顯示 `definition_en`，翻面後顯示 `word_zh`。`--prefetch-cefr-zh` 的缺漏追蹤亦拆分為 `missingZhWords`（無 definition_zh）與 `missingWordZhWords`（無 word_zh）兩個獨立陣列，各自獨立追蹤，互不影響。

- **WSD 腳本改用 `transformers` 直接推論，不透過 `sentence-transformers` 的 `encode()` 包裝層**（`scripts/wsd/glossbert_wsd.py`，2026-06-16）：`tokenizers` 0.20+ 的 Rust 層 `encode_batch` 型別驗證變嚴，與 `transformers` 4.46+ / `sentence-transformers` 3.x 的編碼路徑不相容（`(str, None)` tuple 被拒）。改用 `AutoTokenizer.from_pretrained(MODEL_NAME, use_fast=False)`（Python 版 slow tokenizer）+ `AutoModel` 直接推論，選 CLS token embedding（BGE 系列設計）再做 L2 正規化，等同 cosine similarity。`use_fast=False` 是關鍵：fast tokenizer 走 Rust 路徑，slow tokenizer 走純 Python 路徑，後者對任何合法 Python `str` 均能正確處理，不受 tokenizers 版本影響。`requirements.txt` 同步從 `sentence-transformers>=2.2.2` 改為 `transformers>=4.30.0`，deps 更精簡（sentence-transformers 本身亦依賴 transformers，不需重複）。嵌入品質與原設計相同：`bge-base-en-v1.5` CLS token + `BGE_QUERY_PREFIX` 前綴（sentence 側）+ 無前綴（shortdef 側）的非對稱 retrieval 設定保持不變。

- **`word-def.db` key 設計：hash 英文定義而非例句，讓同一 MW 定義跨書共用翻譯**（`src/nlp/wordDefDb.ts`，2026-06-16）：key = `word:pos::fnv1a(definition_en)`。選 hash 英文定義（而非例句）的原因：① MW API 對同一查詢回傳穩定一致的 shortdef，`definition_en` 不隨書或語境變動；② `definition_zh` 是對「這個英文定義」的翻譯，和哪本書的哪個例句無關——同一個英文定義在任何書裡的中文翻譯都應相同；③ 同字同詞性但不同詞義（如 `bank:noun` 的「金融機構」vs「河岸」）MW 回傳不同 shortdef，hash 後自然得到不同 key，無需額外去重邏輯。相較之下，WSD cache 的 key 用 `fnv1a(sentence)` 是因為 WSD 結果與語境相關；word-def.db 的語意是「這個英文定義的中文翻譯」，與語境無關，因此 hash 的是定義而非例句。`source` 欄記錄翻譯來源（如 `deepl`），方便日後按來源品質篩選或覆蓋。
