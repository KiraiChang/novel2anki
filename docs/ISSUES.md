...
- **`needTranslation` 過濾只看 `definition_zh`，導致 `context_sentence_zh` 永遠空白**（`src/csv/beginnerTranslator.ts`，**已修復 2026-06-10 / 2026-06-11 / 2026-06-12**）：過濾條件現為四欄 OR：`definition_en` OR `definition_zh` OR `context_sentence_zh` OR `word_zh` 任一空白即納入處理，四者皆有才跳過；Phase 3 新增 `definition_en` 回寫（僅從 API 取得時寫入，CSV 原本有值不覆寫）；`definition_zh` 寫入改為空白才寫入（不覆蓋既有值）；Phase 2.5 新增 `word_zh` 批次翻譯（lemma 直接翻譯，獨立於定義+例句批次）。
