  ...
  .action(async (pdfFile: string, options: { // pdfFile = general input file (pdf, epub, csv or directory)
    ...
    if (isDirectory || ext === '.csv') {
      ...
      if (beginnerWordsCsvs.length > 0) {
        ...
        // 強制覆寫快取：CSV → cache（單向，不補填 CSV）
        if (options.pushCache) {
          const pushConfig = parseFillDefZhLayers(options.pushCache);
          const wc = getWordCache();
          console.log('');
          console.log(chalk.cyan(`強制覆寫快取（CSV → cache）`));
          console.log(chalk.gray(`  快取路徑：${wc.cacheZhFilePath}`));
          if (pushConfig.domain) console.log(chalk.gray(`  domain 快取：domain_${pushConfig.domain}_cache_zh.json`));
          if (pushConfig.book)   console.log(chalk.gray(`  book 快取：book_${pushConfig.book}_cache_zh.json`));
          console.log('');
          let totalPushed  = 0;
          let totalSkipped = 0;
          let totalNoMatch = 0;
          for (const csvPath of beginnerWordsCsvs) {
            process.stdout.write(chalk.yellow(`正在處理 ${path.basename(csvPath)}...\n`));
            const result = pushCsvToCache(csvPath, (cur, total) => {
              process.stdout.write(`\r  ${String(Math.round(cur / total * 100)).padStart(3)}% (${cur}/${total})   `);
            }, pushConfig);
            process.stdout.write(`\r${chalk.green(`  ✓ 覆寫 ${result.pushed} 筆 | 空欄略過 ${result.skipped} 筆 | 無 lemma ${result.noMatch} 筆`)}\n`);
            totalPushed  += result.pushed;
            totalSkipped += result.skipped;
            totalNoMatch += result.noMatch;
          }
          console.log('');
          console.log(chalk.green(`完成：共覆寫 ${totalPushed} 筆 | 空欄略過 ${totalSkipped} 筆 | 無 lemma ${totalNoMatch} 筆`));
          console.log(chalk.gray(`word-cache-zh.json：${getWordCache().cacheZhSize} 筆`));
          return;
        }
